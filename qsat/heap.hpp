...
public:
  ...
  bool in_heap(int v) {
    ...
    // just check indices[var]
		...
		// CAUTION: 
		// due to the Literal interface we have
		// we're resizing our heap and clause size while
		// we read in the files
		// so when we run this in_heap check, 'indices' is
		// not resized yet, and we end up accessing invalid
		// indices
		//
